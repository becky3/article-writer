...
1. **ジャーナル一覧の取得**
   - `rag_list_recent(source_type="journal", limit=50)` — RAG に取り込まれたジャーナル一覧（複数リポジトリ横断）
   - 必要に応じて `filters="repository=<リポジトリ名>"` で絞り込む
   - 各ソースの Source 値（source_id）を控え、この時点では本文を取得しない
...
4. **ファイル出力**
   - `.tmp/zenn-drafts/{topic-slug}.md` に保存
   - `.tmp/zenn-drafts/published.txt` に日付・タイトル・ソース参照を追記
...
6. **結果表示**
   ...
   ```text
   ✅ 記事を生成しました: .tmp/zenn-drafts/rss-date-parsing.md
   ```
...
- ジャーナル: `journal:20260217-xxx`

`source_id` は `rag_list_recent` / `rag_search` の結果に含まれる Source 値をそのまま使用する。
