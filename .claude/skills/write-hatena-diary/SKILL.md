...
過去記事を読み、今回の執筆改善につながる点を自分の言葉でメモする（本スキル内で別スキル `/review-hatena-diary` を呼ぶわけではない。Phase 4-6 の自動レビューとは別概念）。メモは Phase 4-4 の執筆時にコンテキストとして参照する（ファイル保存はしない）。
...
続いて本文を書く。**Phase 3-2 でメモした改善点を今回の執筆に反映する**。
