...
1. 既存記事フロントマターから直近 5 件の `pattern`（ID）を読む（例: `grep -h "^pattern:" articles/hatena/*.md | tail -5` で ID のみ抽出。本文は読まない。`archive/` は対象外。記事が無ければ除外なし）
2. パターン ID 一覧（A〜J）から、手順 1 の直近 5 件を除いた集合を作る
