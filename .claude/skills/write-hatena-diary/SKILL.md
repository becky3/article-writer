...
Phase 1（引数パース）で対象日リストを確定し、Phase 2 でジャーナルが存在する日を特定、Phase 3 で共通リソースを読み込んだ後、ジャーナル存在日ごとに Phase 4 を **1 日 = 1 タスクとして繰り返す**。
各日のループ（Phase 4-1〜4-6）は素材取得 → 事実確認 → 記事生成 → レビューまでを独立して完結させる。ループ終了後に Phase 5（後処理）を 1 回実行する。
...
### Phase 3: ループ前準備（共通リソース読み込み + 過去記事メモ）

本 Phase は **Phase 4 のループ全体で 1 回のみ実行**する（サブセクション Phase 3-1 / 3-2 のいずれも各日のループ内で再実行しない）。

#### Phase 3-1: 共通リソース読み込み

執筆ガイド・テンプレートを読み込む。`quality-guidelines.md` / `narrative-guidelines.md` / `template-diary.md` を Read する。読み込んだ内容は Phase 4-4（執筆ガイド + テンプレート参照）で共通利用する。

#### Phase 3-2: 過去記事を読んで今回の執筆改善点をメモする

過去記事を読み、今回の執筆改善につながる点を自分の言葉でメモする（本スキル内で別スキル `/review-hatena-diary` を呼ぶわけではない。Phase 4-6 の自動レビューとは別概念）。
メモは `.tmp/hatena-improvement-memo/<MEMO_NAME>.md` に Write する。Phase 4-4 で同ファイルを Read して執筆に反映する。

`<MEMO_NAME>` は対象期間に応じて決める:

- `MODE=single` / `MODE=auto-next`（単一日対象）: `<記事日付YYYY-MM-DD>`（例: `.tmp/hatena-improvement-memo/2026-05-13.md`）
- `MODE=range`（範囲指定）: `<DATE_FROM>_<DATE_TO>`（例: `.tmp/hatena-improvement-memo/2026-05-10_2026-05-13.md`）

ディレクトリが存在しなければ作成する。同名ファイルが既にある場合は上書きする（再実行時は最新のメモで上書きされる）。`.tmp/` はリポジトリの `.gitignore` 対象のため commit に含まれない。

対象範囲:

- `articles/hatena/` 直下の `YYYY-MM-DD-*.md` のみ。`articles/hatena/archive/` は除外する
- ファイル名先頭 10 文字（`YYYY-MM-DD`）で日付降順ソートし、最新 12 本を Read する。日付抽出の振る舞い（先頭 10 文字が日付形式でない・存在しない日付はスキップ）は Phase 1 の `articles/hatena/` 日付抽出テーブルと同じ規則を準用する
- 過去記事が 12 本未満の場合は存在する全件を対象とする（0 件でもエラーにせず、メモなしで続行する）

メモの観点・軸は本スキルでは指定しない。書き手が読みながら自由に「今回の執筆で改善できる点」を見つけて言葉にする。チェック項目化・観点列挙はしない（指標化すると指標だけ最適化される Goodhart 化を避け、書き手判断の自由を残すため）。
...
続いて本文を書く。**Phase 3-2 で `.tmp/hatena-improvement-memo/<MEMO_NAME>.md` に書き出したメモを Read し、その改善点を今回の執筆に反映する**。
Phase 3-2 で読んだ過去記事の表現・展開・締め方をそのまま再利用してはならない（焼き直し・既視感の回避）。
過去記事は改善方向に上書きするための参照であり、書き写すための参照ではない。
Phase 3-2 の対象（最新 12 本）以外の `articles/hatena/**`・`articles/hatena/archive/**` は本文執筆時に参照しない。
素材（ジャーナル・Bluesky）と物語世界の設定（`narrative-guidelines.md`）を主軸に、Phase 3-2 のメモを改善方向の指針として組み入れて新規に書く:
...
進行パターン ID は Phase 4-4 でフロントマターに `pattern: <ID>` として書き込み済み。patterns.jsonl は使わない（published.jsonl への転記は Phase 4-4 参照）。
...
| 特定日で Bluesky 検索結果 0 件 | Phase 4-3 をスキップ + 当該記事の Bluesky セクションを省略して続行 |
