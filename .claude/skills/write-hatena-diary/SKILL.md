...
3. ディレクトリが存在しなければ作成
4. **新規作成のため Write を使用**（後続 Phase 6 の markdownlint 修正は Edit を使用）。最初から最終稿の場所で編集する（中間ドラフトを別ディレクトリに置かない）
...
1. `GENERATED_PATHS` 全てに対して `npx markdownlint-cli2@0.20.0 <パス1> <パス2> ...` を一括実行
2. エラーがあれば Claude が該当記事を Edit で修正して再実行する。**修正試行を最大 3 回繰り返す**（「修正 → 再実行」を 3 回までループ）。3 回試行後もエラーが残っていれば警告 + 完了メッセージ（記事ファイルは残す）
3. `code "<絶対パス1>" "<絶対パス2>" ...` でまとめてエディタを起動（PATH 不通時はサイレントスキップ）
4. 完了メッセージ。単一記事の場合:
...
| Phase 3〜5 ループ中に MCP タイムアウト・致命エラーで全体中断 | それまでに Write 済みのパスを `GENERATED_PATHS` に残し、Phase 6（markdownlint・エディタオープン・完了メッセージ）を残存分に対して実行する。完了メッセージで中断した旨を併記 |
| markdownlint 修正試行 3 回繰り返してもエラー | 警告表示 + 完了メッセージ（記事ファイルは残す） |
