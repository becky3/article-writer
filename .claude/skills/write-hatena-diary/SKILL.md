...
1. `Skill` ツールで `/review-hatena-diary <パス>` を起動する。`<パス>` には Phase 5 で Write した記事のリポジトリルートからの相対パス（例: `articles/hatena/2026-05-13-diary.md`）を指定する。`/review-hatena-diary` は絶対パス指定をエラー停止するため必ず相対パスで渡す
