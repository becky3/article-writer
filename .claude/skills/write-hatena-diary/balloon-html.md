...
マーカーはキャラ名直結（`:::kuro-chan` / `:::nee-san`）。CSS クラス名は `balloon-l` / `balloon-r` のまま（変換器がマッピングする）。
...
- Markdown 風の文字装飾（変換時に対応 HTML タグへ自動置換される）:
  ...
  - 装飾 3 種いずれも `<code>...</code>` の **外側** にのみ作用する。`` `**foo**` `` `` `*foo*` `` `` `~~foo~~` `` のようにインラインコード内に装飾文字を書いた場合は `<code>**foo**</code>` 等としてリテラル維持される
  - アンダースコア形式（`__bold__` / `_italic_`）は非対応（日本語混じり文で誤爆しやすいため）
  - `***foo***`（bold+italic 同時指定の 3 連続アスタリスク）は **使用禁止**。bold と italic の正規表現が順に当たって `<strong><em>...</strong></em>` という閉じタグ順序の壊れた HTML を生成するため。強調と斜体を同時に当てたい場合は `<strong><em>foo</em></strong>` を直書きする
...
| 条件 | 理由 |
|---|---|
| `:::kuro-chan` / `:::nee-san` / `:::bluesky` の閉じ `:::` がないままファイル末尾に到達 | 未閉鎖ブロック |
| `:::bluesky` ブロックで必須キーが欠落している | 埋め込み URL を組み立てられない |
| ブロック内に別のブロック開始（`:::kuro-chan` 等）が現れた | 入れ子は未サポート |
...
### 3. balloon 内のリポ名・コード断片・文字装飾は通常の Markdown 記法で書く

balloon ブロック内では HTML 直書きとして扱われるため Markdown 構文は素通しでは効かないが、**変換スクリプトが balloon 内の `` `name` `` を `<code>name</code>` に、`**bold**` を `<strong>bold</strong>` 等に自動置換する**。
書き手は balloon の **内外を問わず** 通常通り `` `agent-commons` `` `**重要**` の形で書ける。

✅ 良い例（balloon の内外で同じ書き方）:

```
:::kuro-chan
`agent-commons` のルール書き直しが入りました。**ここが重要** です。
:::

通常段落でも `agent-commons` `**重要**` のように書ける。
```
...
2. balloon ブロック内に `:::bluesky` `:::kuro-chan` `:::nee-san` が混ざっていないか確認
