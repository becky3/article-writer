...
6. **結果表示**
   ...
   - チャットへ完了メッセージを出す直前に、生成ファイルを `code "<絶対パス>"` でエディタにて自動オープンする
     - 引数は必ず引用符で囲む（パスにスペース・特殊文字が含まれても安全に渡すため）
     - 行番号指定（`-g`）は使わない（記事は最初から先頭で確認する想定）
     - `code` コマンドが PATH に通っていない・起動に失敗した場合でもエラー停止せず、後続の結果表示メッセージは続行する
   - チャットへ以下のメッセージを出力する
...
- **観点別並列レビューの推奨**: 公開前の最終確認として `/multi-perspective-review <記事ファイルパス>` の実行を推奨する。デフォルト 5 観点（構成・主題整合性 / 冗長性・重複 / 読者視点 / 文体一貫性 / ガイドライン準拠チェック）で並列レビューが走る。本スキルからの自動呼び出しはせず、ユーザー判断で起動する
