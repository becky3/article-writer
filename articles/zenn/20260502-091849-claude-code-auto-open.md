...
Claude Code は Bash でシェルコマンドを実行できる。VS Code が PATH に通っていれば、Claude に `code <パス>` を実行させるだけでファイルがエディタに開く。

本記事では、Claude にエディタを開かせる基本パターン（チャット指示・ルールへの組み込み・AI-DLC との相性）と、知っておくと役立つ `code` のオプションをまとめる。

## Claude にエディタを開かせる

`code` の実行はチャットから依頼できる。たとえば Claude が
