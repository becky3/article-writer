...
- **動作仕様の詳細**（Phase 構成・素材取得・記法ポリシー・記事構成）: `.claude/skills/write-hatena-diary/SKILL.md` および同階層の関連ファイル（`quality-guidelines.md` / `template-diary.md` / `balloon-html.md`）に保持する
...
- **観点**: 3 観点固定（観点引数による絞り込みなし）
  ...
  - ガイドライン準拠（`quality-guidelines.md` Part 2 機械照合 + 著作物・IP 検出）
...
- `.claude/skills/write-hatena-diary/quality-guidelines.md`: 日記執筆ガイド（`/review-hatena-diary` のガイドライン準拠観点もこれを参照する）
