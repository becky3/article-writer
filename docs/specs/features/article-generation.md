...
| `/review-hatena-diary` | Claude Code セッション内のスラッシュコマンド（自動 / 手動） | はてなブログ向け日記記事を 4 観点（物語ガイド遵守 / ガイドライン準拠 / 言及正確性 / 対話整合性）で並列レビューし、指摘を triage 経由で確認・修正適用する。`/write-hatena-diary` から自動呼び出しされる |
...
はてなブログ向け日記記事を日記特化の 4 観点で並列レビューするスキル。
...
- **観点**: 4 観点固定（観点引数による絞り込みなし）
  - 物語ガイド遵守（一人称・口調等のキャラ整合＋解説・地の文・タイトルの書き方など `narrative-guidelines.md` の遵守）
  - ガイドライン準拠（`quality-guidelines.md` 全節の機械照合 + 著作物・IP 検出）
  - 言及正確性（リポ名・スキル名・事実関係の素材ジャーナル照合）
  - 対話整合性（問いと返しの主語・経路がジャーナル原文と噛み合うか）
- **処理**: 4 観点を `general-purpose` サブエージェントで並列起動 → 結果集約 → `/triage` 連携で指摘を確認 → 確定した修正方針を対象記事へ Edit で適用
...
- `.claude/skills/write-hatena-diary/narrative-guidelines.md`: 日記の物語ガイド（書き手ペルソナ・キャラクター・関係性、`/review-hatena-diary` 観点 1「物語ガイド遵守」が参照）
- `.claude/skills/write-hatena-diary/template-diary.md`: 日記テンプレート・記法ポリシー
...
- `.claude/skills/review-hatena-diary/SKILL.md`: `/review-hatena-diary` スキル本体（日記向け 4 観点並列レビュー + triage 連携）
